...
| `google-calendar-webhook` | **1** | **⚠️ 2026-08-24: the Step 2 REWRITE is built + fully reviewed on branch `claude/last-session-review-f7tqxw` (registry, token-hash auth, revision-guarded date-only inbound writes, deletion exceptions; branch pgTAP 147/147) but NOT DEPLOYED — awaiting Matt's prod apply + deploy + probe. The row below describes the still-deployed v1 spike.** **v2 Task 5B Step-1 SPIKE SCAFFOLD, deployed 2026-08-20 — NOT an implementation.** Exists to prove Google push → Supabase edge function works; it does. Zero DB access by design, no token persistence/verification, no `jobs` writes — all Step 2's contract. Two disjoint paths with **different auth by necessity**: a request carrying `X-Goog-Channel-ID` is Google's notification (Google sends NO `x-webhook-secret`; its auth is the channel token) and always 200s — a non-2xx makes Google retry then kill the channel; everything else is an admin action (`ping`/`register`/`stop`) gated on `x-webhook-secret`. Deployed `--no-verify-jwt` (Google push carries no JWT) — **same readback invariant as `ghl-job-webhook`**. |
...
🔴 **THE SLACK BOT IS NOT A MEMBER OF THE CREW CHANNELS — discovered live 2026-08-20, OPEN, blocks
the Phase 1 gate.** The 5A live probe failed with `Slack post failed: not_in_channel` on Crew 4.

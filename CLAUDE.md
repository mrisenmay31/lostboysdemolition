...
tool handles anything more sensitive than internal estimate drafts. **⚠️ Session 14 (2026-08-27)
BUILT that protection for the financial surface — owner auth (magic link, invite-only) gating all
`/jobs/*` routes + the `/` flip — on branch `claude/v2-task8a-owner-auth`, NOT merged: this
no-login description remains accurate for PRODUCTION until the 8a gate sequence runs (see the v2
row below and `NEXT_SESSION_PROMPT.md`). The estimator picker surface stays no-login by design
even after 8a merges; once it deploys, `NEXT_PUBLIC_SUPABASE_ANON_KEY` becomes load-bearing.** **Manual Setup #2 (provision 3

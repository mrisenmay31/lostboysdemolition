...
## Where the project actually stands (2026-07-30 / updated 2026-08-13 / checkpoint 2026-09-02)

**⚠️ 2026-09-02 ADOPTION CHECKPOINT (Session 16, docs-only) — read
`docs/superpowers/plans/2026-09-02-adoption-checkpoint-assessment.md` before planning anything.**
Roughly half the v2 program is built, reviewed, and live, and **the business has not used any of
it**: 0 real estimates in the app vs 64 real Fillout estimates created 2026-07-31→2026-09-02
(Airtable IDs 297→360), 0 real jobs, Dane never invited, no foreman accounts, Slack bot not in the
crew channels. Matt ruled: adoption sprint first; **the profitability dashboard IS the product**;
bare bones first, manual actuals acceptable, automate later.
**Session 17 (2026-09-02, docs-only) RULED everything and approved the next build — read the
BUILD_PLAN `AMENDED 2026-09-02` block.** The **estimate builder redesign builds FIRST** (Session 18):
spec `docs/superpowers/specs/2026-09-02-estimate-builder-redesign-design.md`, plan
`docs/superpowers/plans/2026-09-02-estimate-builder-redesign.md`, approved mockup
`docs/prototypes/2026-09-02-estimate-builder-approved.html`. Then 8b as the picker-side
"Mark started / Mark completed" action (`docs/superpowers/plans/2026-09-02-v2-task8b-status-action-staged.md`).
Owner auth goes dormant (`OWNER_AUTH_ENABLED`, default off) inside the redesign build; quotes
reach customers as itemized GHL estimate documents; v2 Phases 4–6 are frozen as backlog.
The paragraphs below describe the pre-checkpoint history and remain accurate.
...
| **Profitability Program v2** | 🟢 **Phase 0 (Tasks 0A + 0B) COMPLETE 2026-08-18.** Canonical program `docs/superpowers/plans/2026-08-18-live-job-profitability-health-dashboard-v2.md` (Tasks 0A/0B + 1–17, phases 0–6) — absorbs/re-specifies C, E, F, G and Phase D1. Version 1 archived. Task 0A: `docs/runbooks/profitability-schema-validation.md` written + CLAUDE.md/BUILD_PLAN.md doc corrections landed. Task 0B: `workforce_profiles` BL-7 boundary migration **APPLIED TO PRODUCTION** (branch red/green → production dry-run red/green → real apply, all live-verified). **BL-7 is CLOSED.** **Phase 1 Sessions 1–3 SHIPPED 2026-08-19** on branch `claude/last-session-review-f7tqxw` (NOT merged to main — Matt decides when): v2 Task 1 schema (14 enums, 16 tables, 12 `jobs` columns), Task 2 economics + commercial lifecycle (`create_estimate_with_items_v2`, identity links/presentations/acceptance events+state, deviation-12 `accepted_price` pinned at acceptance), and Task 4 `schedule_estimate` (the canonical app-side job-minting RPC) are **ALL APPLIED TO PRODUCTION** (migration head `20260819191046`, 32 applied). Task 3 forecast engine + the Task 2/4 web work (economics inputs, present/accept/reverse lifecycle UI, `/estimates/[id]/schedule`) are on the branch, NOT yet on production Vercel. `ghl-job-webhook` v20 deployed with the cutover flag UNSET — legacy minting unchanged; the permanent `ENABLE_GHL_ACCEPTANCE_JOB_CREATION=false` flip happens ONLY at the Phase 1 gate. **Session 4 SHIPPED 2026-08-20:** Task 5A (outbound dispatcher) — `claim_integration_events` + `cancel_scheduled_job` + `*/5` cron **APPLIED TO PRODUCTION** (head `20260820152300`, 35 applied), `integration-dispatcher` v1 **DEPLOYED** (`verify_jwt=false` read back), `scheduleActions.ts` on the branch; branch-validated pgTAP 65/65, suites deno 371 + web 556. **Session 5 SHIPPED 2026-08-20 (no migrations, no prod applies):** (a) the **Task 5A live TEST-job probe ran** — estimate 1427 → JOB-1105 → dispatcher → Calendar + GHL both directions → cancel → cleanup, all verified; it found the 🔴 Slack bot-membership defect (see Environment Variables) and proved, unprompted, that **Task 4's `app_is_schedule_authority` compat check works in production** (the real GHL Job Scheduled workflow fired into `ghl-job-webhook` and was correctly skipped) — which retires the "two minting paths coexist" risk flag; (b) **Task 5B Step 1, the watch-channel spike, PASSED** — Google push to a Supabase edge function works, no domain verification needed, so 5B does NOT degrade to polling-only. **Sessions 6–8: Task 5B Step 2 built + fully reviewed (2026-08-24), then SHIPPED TO PRODUCTION 2026-08-25** — 3 migrations applied (head `20260825171051`, 38 applied), `google-calendar-webhook` **v2 deployed** via the invariant, cron `calendar-sync-maintenance` live at `7,37 * * * *`. Probe legs 1/2 + echo termination PROVEN LIVE (estimate 1428 → **JOB-1106**, Crew 4, 2026-12-22→23; **first-ever successful dispatcher Slack delivery**, to #ops-test via `SLACK_TEST_CHANNEL_OVERRIDE`). **Session 9 (2026-08-25, same day): probe legs 3/5/6 COMPLETE — all six legs proven live, Task 5B is DONE.** Inbound date apply → rev 2 → dispatcher mirror (update-not-create idempotency proven, R7 re-notify); deletion → exception + alert, job untouched → `dismiss` via RPC → rev 3, crew event recreated; `closed_lost` teardown (M7 rev-share observed live, zero GHL artifacts, re-cancel raise verbatim, both events deleted, echo silent); **override UNSET + confirmed absent.** No estimate burned. **Next = Task 7, the Phase 1 gate. Step 1 (whole-branch adversarial review) DONE 2026-08-25 Session 10 — MERGE-READY after fix round `604ddc5` (0 blocking; repo↔prod proven functionally identical; deferral ledger in the Session 10 BUILD_LOG entry).** **Gate preconditions AMENDED by Matt same session: the phone smoke + real estimate (**first real now ≥1429**; 1426 burned by a failed CHECK, 1427 by the 5A probe, 1428 by the 5B probe), the authenticated JOB-1104 webhook fire, the Slack bot invitations, and the calendar eyeballs (2026-12-15/16, 2026-12-28/29) are ALL BACKLOGGED → BL-8; the gate proceeds without them, and the Step 2 E2E runs Slack via `SLACK_TEST_CHANNEL_OVERRIDE=#ops-test` (unset at close).** **Steps 2+3 DONE same session — THE PHASE 1 GATE IS PASSED: the E2E ran clean (estimate 1429 v1→v2 → JOB-1107, Crew 2; deviation-12 pin verified live; dispatcher attempt-1 everywhere; echo `dates_unchanged` both calendars; reactivation FIRST-PROVEN — cancel → re-schedule new dates → same job, rev 2, no second budget; teardown clean, first real estimate now ≥1430) and the PERMANENT CUTOVER IS LIVE (`ghl-job-webhook` v25, flag=`false`; Quote Accepted mints nothing, live-verified through the real workflow on the 5A TEST opportunity, restored to Closed Lost after). **MERGED TO MAIN same session (Matt's approval): fast-forward `4dd15cc..2085f42` (+ `58928c3` docs); production Vercel deploy VERIFIED READY at https://lostboysdemolition.vercel.app (`/` 307→`/estimates` per the pre-Task-8 decision; `/estimates` and `/jobs/exceptions` 200) — the Phase 1 web code (economics inputs, present/accept/reverse lifecycle UI, schedule flow, exceptions queue) is LIVE in production. TASK 7 CLOSED; v2 PHASE 1 COMPLETE.** **Phase 2 Session 11 (2026-08-26): the ratified prototype decisions were ported into main's v2 plan (`c3d4e48` — no "portfolio"; the locked `payment_processing` capture-only presentation); Dane's prototype feedback still not received (zero artifact comments) — Matt ruled Task 6 proceeds on the ratified decisions. v2 Task 6 BUILT on branch `claude/v2-task6-job-dashboard` (10 commits `824f910..995dc55`, web-only): `/jobs` Job Dashboard (six filters, health cards, Jobs nav; `/` untouched per the Task 8 deferral) + `/jobs/[jobNumber]` profitability detail (locked comparison presentation, honest-null rendering, cancel/postpone UI behind the estimator gate, alert resolution incl. `calendar_watch:*`, exceptions queue filtered to `calendar_deleted`) over new `lib/jobs/map.ts` (pure, exclusion-proof-tested) + `lib/jobs/healthRepo.ts` (watermark-gated `job_forecast_snapshots` persistence; `QUERY_ROW_CAP` truncation sentinel — pagination deferred to Phase C). All 5 task reviews + final whole-branch review clean after one fix wave; web 650/650, deno 411/411 (golden intact), build green; live-verified against production (chain cross-checks to the cent on JOB-1107; snapshot invariant held). **MERGED TO MAIN same session (Matt's go): fast-forward `c3d4e48..5663a46`, pushed; production deploy VERIFIED (~40s): `/jobs` + `/jobs/[jobNumber]` LIVE on the network-open app, `/` still 307→`/estimates` (Task 8 owns the flip). TASK 6 CLOSED.** **Session 12 (2026-08-26, same day): v2 TASK 7 SHIPPED — the manual ledger is LIVE.** Migration `manual_ledger_rpcs` **APPLIED TO PRODUCTION** (MCP version `20260826180811`, head advanced, 39 applied; repo file `20260826150000_manual_ledger_rpcs.sql` is the identical SQL): 4 service-role-only RPCs — `create_job_cost_entry`/`correct_job_cost_entry` (FOR UPDATE + whitelisted patch + same-transaction `job_cost_entry_audit` + `source_revision`/`updated_at` bump; manual-source-only until Task 14)/`create_job_revenue_entry`/`open_category_overrun_alert` (fingerprint `category_overrun:<category>`, watch, dedup on the open partial-unique index, NO Slack — Task 12 owns Slack); both cost RPCs run `mark_job_reconciliation_required` + the overrun check in-transaction. **Locked conventions:** credit/refund stored NEGATIVE (RPC-enforced; forms capture positive, `web/src/lib/ledger/repo.ts` negates — matches `map.ts`'s signed sum); cost amounts strictly positive (correct to adjust, void to remove — no delete path); dates = Denver business dates stored as Denver NOON; attribution = `metadata.entered_by` (`p_actor` always null); RPC raise texts are a cross-lane API matched by substring in `classifyLedgerError` — never reword one side; NO revenue correction path (offsetting credit/refund). Web: `web/src/lib/ledger/` (pure Zod — `z.number()` only, empty-string rejected; repo with `QUERY_ROW_CAP` sentinel), 3 estimator-gated actions, `/jobs/[jobNumber]/costs` + `/revenue` screens. pgTAP 41/41 red→green on a disposable branch (**branches do NOT clone data** — auth.users fixture had to be synthetic); suites web 728/728, deno 411/411 golden intact; 5 task reviews + whole-branch clean (2 review-caught fixes: note-wiping correction, missing row-cap sentinel). **Live smoke on JOB-1107 proved every leg in production** incl. overrun alert ($215 vs $195), note preservation, sign-guard raise, void exclusion, reconcile no-op, snapshot invariant (0 rows). Residue on JOB-1107: 2 voided cost entries, 3 audit rows, 2 net-zero revenue rows, 1 resolved alert (removal = Matt per-item). **MERGED TO MAIN (fast-forward `a0a92a2..0e893aa`, Matt's approval) + production deploy VERIFIED — both routes LIVE. TASK 7 CLOSED; PHASE 2 BUILD COMPLETE.** **Session 13 (2026-08-27): THE PHASE 2 GATE IS PASSED — v2 PHASE 2 COMPLETE.** Full E2E on estimate 1430 → JOB-1108 (Matt phone-driving, first phone-driven probe): all 11 manual facts, two live corrections (note preservation + quantity-only patch through the real UI), comparison table to the cent, 6 predicted overrun alerts, dispatcher attempt-1 both directions, Slack to #ops-test confirmed, clean `closed_lost` teardown, override unset+confirmed, golden 411/411. **Gate finding fixed+deployed mid-gate:** `job_cost_entry_audit` had no UI surface — branch `claude/gate-audit-render` (adversarial review caught a void-vs-amount title-priority defect; review-locked order void > amount > state > generic) merged `01d48a3..91a5531`, deploy verified, rendering live-verified on JOB-1108's real audit rows; web tests 758. **Two findings backlogged (Matt):** overrun alert `action_path` self-links (should be `/jobs/<job>/costs`; one-line RPC migration at next window); costs-edit discoverability. First real estimate now **≥1431**.** **Session 14 (2026-08-27): PHASE 3 OPENED — Task 8 SPLIT 8a/8b (Matt's ruling); 8a (owner auth) + Task 9 (forecast overrides) BUILT AND FULLY REVIEWED on branch `claude/v2-task8a-owner-auth` (11 commits `654d4c4..1b0527a`, worktree `.claude/worktrees/task8a`) — NOT MERGED; production unchanged.** 8a = email magic-link auth (`shouldCreateUser:false` invite-only), owner-gating of every `/jobs/*` financial route (proxy + layout defense in depth), the `/` flip (active owner → `/jobs`, else `/estimates`), owner-promotion runbook `docs/runbooks/owner-promotion.md` (on the branch). Task 9 = locked Zod validation (z.number() only; positive crew/hours = the crew-days zero-divisor guard), append-only `job_forecast_overrides` writes (first non-null `created_by`), the first authenticated-owner-gated server action, ForecastOverridePanel on job detail (replaced the old static overrides block — ruled consolidation). 7 tasks + 2 fix rounds, final whole-branch review READY TO MERGE (0 Critical); suites web 811/811, deno 411/411 golden intact; ZERO migrations; picker surface + `supabase/` diffs byte-empty. ⚠️ Review-caught config dependency: Supabase's DEFAULT email templates never carry `token_hash` — **sign-in fails 100% until the Magic Link + Invite templates are customized per the runbook.** Ruled boundary: `/jobs` PAGES owner-gated; pre-existing cost/revenue/schedule/cancel WRITES stay picker-gated (8b hardening candidate). **Session 15 (2026-08-27, same day): THE 8a GATE SEQUENCE RAN CLEAN — 8a + TASK 9 MERGED TO MAIN AND LIVE IN PRODUCTION.** All six steps on Matt's per-step go: (1) Supabase Auth config done in the dashboard (Site URL, both redirect URLs, **Magic Link + Invite templates customized with the `token_hash` links** — the review-caught 100%-dead-end config item is closed) + Vercel prod env verified (both `NEXT_PUBLIC_SUPABASE_*` present); (2) **owner promotion EXECUTED** — Matt's `workforce_profiles` row is `owner`/active, keyed on `auth_user_id` because the live `display_name` is lowercase `matt` (the runbook's `display_name='Matt'` predicate would have zero-row no-opped; runbook fixed, `d19566d`); (3) non-FF merge `8cfe920..34ab995` pushed after proving the merged `web/`+`supabase/` trees byte-identical to the reviewed branch; (4) deploy verified (~40s): `/` 307→`/estimates` anon, `/jobs` + `/jobs/exceptions` 307→sign-in, **deep-link `/jobs/JOB-1108/costs` Location carries the full `next=` path — the proxy is registered in production**, `/estimates` + `/auth/sign-in` 200; (5) **Matt's phone smoke PASSED all four checks** — after one finding: `matt@ctaintegrity.com` was rejected at sign-in because the sole `auth.users` row (2026-05-05 May test user, the one 0B backfilled and this gate promoted) is **`matt@lostboysdemolition.com` — Matt ruled: that stays the owner sign-in address** (recorded in the runbook); (6) docs closed. Zero migrations, zero edge-function changes, estimator picker surface untouched. **TASK 8a CLOSED; TASK 9 LIVE (forecast overrides, owner-gated). Next = 8b: foreman checklists + the Phase 3 gate + the `action_path` migration.** See the 2026-08-27 Session 15/14/13, 2026-08-26 Session 12/11, 2026-08-25 Session 10/9/8, two 2026-08-20, and three 2026-08-19 `BUILD_LOG.md` entries and the phase plan's live checkboxes. |
| **Estimate builder redesign** | 🟢 **APPROVED 2026-09-02 (Session 17), builds Session 18.** Four-step builder (Job → Client → Financial → Review), server-side drafts, customer lines with editable prices (line sum = quoted price + reason chip), one "Other job costs" field with breakdown, "Profit margin" chips + true margin, dashboard-prototype tokens app-wide (placeholders), `/jobs` re-skinned styling-only, owner auth dormant. One migration (`estimate_line_items.customer_price/is_custom`, `estimates.client_business_name/requested_start_date`, `estimate_drafts` + 3 RPCs, create-v2 line-sum guard). Spec + plan + mockup paths in the checkpoint note above. |
| **2026-09-02 adoption checkpoint** | 🟢 **RULED in Session 17 — see the BUILD_PLAN `AMENDED 2026-09-02` block.** 8b = picker-side status action (staged, builds after the redesign); anyone enters actuals; quotes = itemized GHL estimate docs; engine 7-day status-aware rules; Phases 4–6 frozen; adoption checklist `docs/superpowers/plans/2026-09-02-adoption-sprint-checklist.md`. *Session 16 history:* 🟡 **Session 16 (docs-only): 8b NOT built.** Whole-build assessment found zero real usage (see the checkpoint note at the top of "Where the project actually stands"). Matt ruled "dashboard is the product, bare bones first". Proposal pending his three rulings: owner "Mark started / Mark completed" action instead of the foreman area; freeze v2 Phases 4–6 as backlog; 30 days of real jobs through the manual loop, then automate BILL (Task 14) and time (Task 13); amend v2 to own Track B / calibration loop / Fillout retirement / deposit policy / callbacks / sign-off. Full record: `docs/superpowers/plans/2026-09-02-adoption-checkpoint-assessment.md`. |
...
tool handles anything more sensitive than internal estimate drafts. **✅ 8a SHIPPED 2026-08-27
(Session 15, merge `34ab995`) — the network-open posture now applies ONLY to the estimator
surface.** Live production posture: `/estimates/*` stays no-login with the picker, by design;
**every `/jobs/*` financial route is owner-gated** (email magic link, `shouldCreateUser: false`
invite-only — strangers cannot mint auth users on the open page; enforcement is defense-in-depth:
`web/src/proxy.ts` + the `(app)/jobs/layout.tsx` gate over `workforce_profiles`); `/` routes an
active owner to `/jobs` and everyone else to `/estimates`. **The owner sign-in email is
`matt@lostboysdemolition.com`, not Matt's CTA address** — see `docs/runbooks/owner-promotion.md`
§2 for why, and for Dane's later invite flow. `NEXT_PUBLIC_SUPABASE_ANON_KEY` is now
**load-bearing** (the SSR auth client uses it — no longer "unused but harmless").
**Ruled boundary (Session 14, restated at the gate):** `/jobs` PAGES are owner-only, but the
pre-existing cost/revenue/schedule/cancel server-action WRITES remain picker-gated and
network-invocable — unchanged posture, an 8b hardening candidate. Only the Task 9
forecast-override action is authenticated-owner-gated.** **Manual Setup #2 (provision 3

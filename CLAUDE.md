...
⚠️ **`SLACK_TEST_CHANNEL_OVERRIDE=C0BPPG8997Z` (#ops-test) is currently SET (2026-08-25, for the
5B probe).** While set, ALL crew Slack posts from all three functions redirect to #ops-test —
including `crew-night-before`'s nightly digest. **Unset it (and confirm absent — BL-4 precedent)
when the 5B probe completes**, before any real job is scheduled.

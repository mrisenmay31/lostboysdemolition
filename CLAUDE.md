...
`SLACK_TEST_CHANNEL_OVERRIDE` — **UNSET and confirmed absent 2026-08-25 (Session 9, at 5B probe
close; BL-4 precedent readback).** It was set to #ops-test only for the 5B probe. While set it
redirects ALL crew Slack posts from all three functions (including `crew-night-before`'s nightly
digest); it must never be set in normal operation.

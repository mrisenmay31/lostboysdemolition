...
### 2026-09-02 — Session 17 (docs-only): CHECKPOINT RULED; ESTIMATE BUILDER REDESIGN APPROVED (builds Session 18); 8b staged as the status action; owner auth to go dormant

Session 17 opened on the three checkpoint questions and closed with every one ruled, a fourth build
approved, and nothing shipped. **No code, no deploys, no migrations, no data writes** — every check was
read-only (production Supabase counts unchanged since Session 16: 21 test estimates, max 1430, 6
cancelled jobs, 0 open alerts, 1 workforce row).

**Rulings (Matt), all recorded in BUILD_PLAN `AMENDED 2026-09-02`:** (1) 8b = a "Mark started / Mark
completed" action on job detail (foreman area → backlog); (2) anyone with app access enters actuals →
**owner auth goes dormant** behind `OWNER_AUTH_ENABLED` (default off; the 8a code stays), picker stays
Dane/Jackson/Matt; (3) quotes reach customers as **itemized GHL estimate documents**; (4) engine
manual-phase rules: status-aware freshness (only `in_progress`) at 7 days, completion → forecast =
actuals; (5) **the estimate builder redesign builds FIRST**; (6) v2 Phases 4–6 frozen as backlog,
30-day real-jobs milestone before automating (Task 14, then 13); (7) adoption checklist opened.

**Estimate builder redesign — brainstormed, mocked up, APPROVED.** Driver: "clunky and basic", mobile-first,
match the Job Dashboard look. Finding that mattered: the SHIPPED `/jobs` is plain zinc; the 2026-08-21
**prototype** is the ink/orange system — Matt chose the prototype's tokens (as swappable placeholders with a
logo slot). Data mined from the 321 golden estimates: 320/321 used total hours (days×crew once → dropped
from the UI), dumps median 1.5, job-specific costs on 179/321, markup ∈ {20,25,30,35} on 316/321. Fillout
captures ~16 inputs vs the app's 24 (the six v2 economics fields were the bloat → one "Other job costs"
field with breakdown). Three interactive flow mockups were published and kept (A price-first, B one-screen,
C stepped); Matt chose C and restructured it: **Job → Client → Financial → Review**, scope names the
customer lines, hours/dumps entered once, total bid first then lines with **editable prices** (line sum
becomes the quoted price; reason chip; the existing `quote_override_reason_required` CHECK fires on INSERT
so the reason is required), business name for contractors, dates on Job, **Save as draft on every step →
server-side `estimate_drafts` promoted on finish**. Revision-1 mockup approved with two edits (no dumps
subtext; Review edits move the total). Mockups preserved in `docs/prototypes/2026-09-02-*.html`.
Spec: `docs/superpowers/specs/2026-09-02-estimate-builder-redesign-design.md`. Plan (9 lanes, waves,
file ownership, locked interfaces, gates): `docs/superpowers/plans/2026-09-02-estimate-builder-redesign.md`.

**Data-layer facts the plan rests on (read-only):** `estimate_line_items.scope_library_id` already nullable,
**no price column**; line items fully immutable (prices edit before first save only); `create_estimate_with_items_v2`
does zero line validation (a `customer_price` key is inert until the column exists) — the plan adds a line-sum
guard raise; no `business_name`/`start_date` on `estimates`; immutability fn's watched list must gain both new
columns (precedent `20260814230000`); Geist is loaded but the body hard-codes Arial (font never applied); 17
copies of the currency formatter, 3 `formatStatusLabel`, 3 divergent health maps → `format.ts` + primitives;
`isStale(null)` is true and time/expense legs are unconditional (why every job would read "watch" in a
manual phase — ruling 4); `CancelJobPanel` shows for `in_progress` but the RPC permits `scheduled` only.

**8b staged** (`docs/superpowers/plans/2026-09-02-v2-task8b-status-action-staged.md`): `advance_job_status`
RPC design + raise texts, stage 6→7/7→8 (first writer of 7/8), non-`:rev` idempotency keys, dispatcher
needs only 2 fixture stages + 2 tests, `action_path` fixes ride along, engine rules, `DEFAULT_HOURS_PER_DAY`.

**Next session opens the redesign build** per NEXT_SESSION_PROMPT: worktree `claude/estimate-redesign`, Wave 0
= S1 ∥ D1→D2 ∥ L1→L2→L3 ∥ A1 (5 concurrent), adversarial review per task, Matt's gates for migration/merge/
deploy, then his phone smoke on a real estimate (≥1431) as adoption step 1.

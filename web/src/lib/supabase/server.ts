...
export async function createClient() {
  ...
  if (!url || !anonKey) {
    throw new Error(
      "createClient: NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY must both be " +
        "set (check web/.env.local — see task-6-report.md if it doesn't exist yet).",
    );
  }

...
- [~] **Step 4c (PARTIAL — run 2026-08-20, Session 5):** live TEST-job probe executed against production (estimate 1427 → JOB-1105, Crew 4, 2026-12-15/16). **PROVEN LIVE:** schedule→outbox→cron→dispatcher claim; budget v1 deviation-12 pin; Calendar event *create* (both legs, ids persisted); GHL stage projection both directions (Job Scheduled, then Closed Lost (Declined)) incl. `sync_log app_to_ghl`; retry/backoff (att1→att2); `cancel_scheduled_job` fact preservation; calendar-id clearing on `job.cancelled`; re-cancel raise + `classifyCancelError` needle match. **Bonus:** the real GHL Job Scheduled workflow fired into `ghl-job-webhook` and Task 4's `app_is_schedule_authority` compat check skipped it correctly — first production execution of that guard, retiring the "two minting paths coexist" risk flag.
  **🔴 BLOCKING DEFECT FOUND — the Slack bot is not a member of the crew channels** (`Slack post failed: not_in_channel`, Crew 4). System history shows exactly ONE successful Slack post ever, to Crew 1 on 2026-08-13; crews 2/3/4 never exercised. Unit tests inject a fake `postSlackMessage` and `crew-night-before` has skipped nightly since 2026-08-14, so nothing could have caught it. **Matt must invite the bot to the Crew 1–4 channels before the Phase 1 gate.**
  **Still unproven, carried into the gate:** crew Slack delivery; Calendar event *deletion* (needs Matt's eyeball on 2026-12-15/16 — no calendar read access in-session, and `deleteCalendarEvent` treats 404/410 as success); calendar update-not-create idempotency (blocked behind Slack); dead-letter + `job_alerts` (operator forced the row succeeded on Matt's instruction, ~3 attempts short). `postponed` cancel resolution also unexercised — deliberately, since it returns GHL to Quote Accepted and would trip the still-live legacy minting workflow; probe it only after the gate flag flip.

### Task 6 (Session 5 — v2 Task 5B): Inbound Calendar sync — spike first

- [x] **Step 1 (SPIKE) — ✅ PASSED 2026-08-20. GOOGLE PUSH TO A SUPABASE EDGE FUNCTION WORKS. 5B is GO; the reconciliation-polling-only fallback is NOT needed.**
  Stub `google-calendar-webhook` **v1 deployed** `--no-verify-jwt` (readback `verify_jwt: false` ✓; `ghl-job-webhook` held at v20 with unchanged sha, `integration-dispatcher` unchanged — no collateral bumps). Secret-less probe → function-level `{"error":"Unauthorized"}` 401; a POST carrying `X-Goog-Channel-ID` and no Supabase auth → 200, so Google's JWT-less notification is accepted.
  `events.watch` on the main calendar returned **HTTP 200** — channel `spike-5b-fd74753fa7b3`, resource `dkMnfSltyPXxmK7Q1IZDuHMo2XI` — and Google delivered a real `sync` notification **0.3 s later**, with `X-Goog-Channel-Token` round-tripped intact. Channel stopped afterwards via `channels.stop` (204); no live channel left behind.

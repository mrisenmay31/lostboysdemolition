...
10. **`mark_job_reconciliation_required`'s outbox `idempotency_key` is job-scoped** (`<job_number>|reconcile:<kind>:<id>`), deviating from the spec's literal "same idempotency key" — Session 1 review *demonstrated* that a globally-unique key silently drops the Slack event when the same source recurs on another job or after an alert resolution, while the alert's dedup is per-job/per-open-window. The `job_alerts` fingerprint itself stays spec-exact.
11. **`job_forecast_overrides` gains `check (category is distinct from 'direct_labor' or expected_remaining_cost is null)`** — the forecast engine ignores a labor `expected_remaining_cost` by contract (labor ETC always flows from the crew-days/remaining-hours model); this CHECK stops Task 9 from persisting a legal-but-silently-ignored row.
